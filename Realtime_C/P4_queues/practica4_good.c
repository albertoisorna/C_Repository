...
int main(int argc, char **argv) {
   ...
   pthread_sigmask(SIG_BLOCK, &spuls, NULL);
    ...
    // 1. añadimos permiso lectura/escritura
     	...
     	id = fork();
      	if(id == 0) {
        	...
        	sprintf(arg1, "%2.2f", (float)FILA_APARCAMIENTO); 
         	...
         	sprintf(arg3, "%d", i); 
         	//2. falta pasar la tabla de datos de cada vehículos
...
int ejec_peticion(struct msg_pet peticion) {
	...
	respuesta.tipo = tipo;
 	...
 	if(responder) { 
		// 7. El mensaje es de respuesta no peticion
